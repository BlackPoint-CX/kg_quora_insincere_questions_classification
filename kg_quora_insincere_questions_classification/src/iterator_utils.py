...
def build_train_iterator(train_dataset, vocab_table, batch_size, buffer_size=None, num_parallel_calls=4):
    ...
    train_dataset = train_dataset.map(
        map_func=lambda idx, target, question_text: (
            idx, tf.string_split([question_text]).values, target),
    ...
    train_dataset = train_dataset.map(map_func=lambda idx, token_ids, label: (idx, token_ids, tf.size(token_ids), label))
